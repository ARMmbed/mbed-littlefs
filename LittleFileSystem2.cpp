...
{
    _mutex.lock();
    LFS2_TRACE("mount(%p)", bd);
    _bd = bd;
    int err = _bd->init();
    if (err) {
        _bd = NULL;
        LFS2_TRACE("mount -> %d", err);
        _mutex.unlock();
        return err;
    }
    ...
    if (err) {
        _bd = NULL;
        LFS2_TRACE("mount -> %d", lfs2_toerror(err));
        _mutex.unlock();
        return lfs2_toerror(err);
    }

    _mutex.unlock();
    LFS2_TRACE("mount -> %d", 0);
    return 0;
}
...
int LittleFileSystem2::format(BlockDevice *bd,
                             ...
                             lfs2_size_t cache_size, lfs2_size_t lookahead_size)
{
    LFS2_TRACE("format(%p, %ld, %ld, %ld, %ld)",
             bd, block_size, block_cycles, cache_size, lookahead_size);
    int err = bd->init();
    if (err) {
        LFS2_TRACE("format -> %d", err);
        return err;
    }
    ...
    if (err) {
        LFS2_TRACE("format -> %d", lfs2_toerror(err));
        return lfs2_toerror(err);
    }
...
{
    _mutex.lock();
    LFS2_TRACE("remove(\"%s\")", filename);
    int err = lfs2_remove(&_lfs, filename);
    LFS2_TRACE("remove -> %d", lfs2_toerror(err));
    _mutex.unlock();
    return lfs2_toerror(err);
}
...
{
    _mutex.lock();
    LFS2_TRACE("rename(\"%s\", \"%s\")", oldname, newname);
    int err = lfs2_rename(&_lfs, oldname, newname);
    LFS2_TRACE("rename -> %d", lfs2_toerror(err));
    _mutex.unlock();
    return lfs2_toerror(err);
}
...
{
    _mutex.lock();
    LFS2_TRACE("mkdir(\"%s\", 0x%lx)", name, mode);
    int err = lfs2_mkdir(&_lfs, name);
    LFS2_TRACE("mkdir -> %d", lfs2_toerror(err));
    _mutex.unlock();
    return lfs2_toerror(err);
}
...
int LittleFileSystem2::stat(const char *name, struct stat *st)
{
    ...
    _mutex.lock();
    LFS2_TRACE("stat(\"%s\", %p)", name, st);
    int err = lfs2_stat(&_lfs, name, &info);
    LFS2_TRACE("stat -> %d", lfs2_toerror(err));
    _mutex.unlock();
...
int LittleFileSystem2::statvfs(const char *name, struct statvfs *st)
{
    ...
    _mutex.lock();
    LFS2_TRACE("statvfs(\"%s\", %p)", name, st);
    in_use = lfs2_fs_size(&_lfs);
    LFS2_TRACE("statvfs -> %d", lfs2_toerror(in_use));
    _mutex.unlock();
    if (in_use < 0) {
        return in_use;
    }
...
int LittleFileSystem2::file_open(fs_file_t *file, const char *path, int flags)
{
    ...
    _mutex.lock();
    LFS2_TRACE("file_open(%p, \"%s\", 0x%x)", *file, path, flags);
    int err = lfs2_file_open(&_lfs, f, path, lfs2_fromflags(flags));
    LFS2_TRACE("file_open -> %d", lfs2_toerror(err));
    _mutex.unlock();
    if (!err) {
        *file = f;
    } else {
...
int LittleFileSystem2::file_close(fs_file_t file)
{
    ...
    _mutex.lock();
    LFS2_TRACE("file_close(%p)", file);
    int err = lfs2_file_close(&_lfs, f);
    LFS2_TRACE("file_close -> %d", lfs2_toerror(err));
    _mutex.unlock();
    delete f;
    return lfs2_toerror(err);
}
...
ssize_t LittleFileSystem2::file_read(fs_file_t file, void *buffer, size_t len)
{
    lfs2_file_t *f = (lfs2_file_t *)file;
    _mutex.lock();
    LFS2_TRACE("file_read(%p, %p, %d)", file, buffer, len);
    lfs2_ssize_t res = lfs2_file_read(&_lfs, f, buffer, len);
    LFS2_TRACE("file_read -> %d", lfs2_toerror(res));
    _mutex.unlock();
...
ssize_t LittleFileSystem2::file_write(fs_file_t file, const void *buffer, size_t len)
{
    lfs2_file_t *f = (lfs2_file_t *)file;
    _mutex.lock();
    LFS2_TRACE("file_write(%p, %p, %d)", file, buffer, len);
    lfs2_ssize_t res = lfs2_file_write(&_lfs, f, buffer, len);
    LFS2_TRACE("file_write -> %d", lfs2_toerror(res));
    _mutex.unlock();
    return lfs2_toerror(res);
}
...
int LittleFileSystem2::file_sync(fs_file_t file)
{
    lfs2_file_t *f = (lfs2_file_t *)file;
    _mutex.lock();
    LFS2_TRACE("file_sync(%p)", file);
    int err = lfs2_file_sync(&_lfs, f);
    LFS2_TRACE("file_sync -> %d", lfs2_toerror(err));
    _mutex.unlock();
    return lfs2_toerror(err);
}
...
off_t LittleFileSystem2::file_seek(fs_file_t file, off_t offset, int whence)
{
    lfs2_file_t *f = (lfs2_file_t *)file;
    _mutex.lock();
    LFS2_TRACE("file_seek(%p, %ld, %d)", file, offset, whence);
    off_t res = lfs2_file_seek(&_lfs, f, offset, lfs2_fromwhence(whence));
    LFS2_TRACE("file_seek -> %d", lfs2_toerror(res));
    _mutex.unlock();
    return lfs2_toerror(res);
}
...
off_t LittleFileSystem2::file_tell(fs_file_t file)
{
    lfs2_file_t *f = (lfs2_file_t *)file;
    _mutex.lock();
    LFS2_TRACE("file_tell(%p)", file);
    off_t res = lfs2_file_tell(&_lfs, f);
    LFS2_TRACE("file_tell -> %d", lfs2_toerror(res));
    _mutex.unlock();
    return lfs2_toerror(res);
}
...
off_t LittleFileSystem2::file_size(fs_file_t file)
{
    lfs2_file_t *f = (lfs2_file_t *)file;
    _mutex.lock();
    LFS2_TRACE("file_size(%p)", file);
    off_t res = lfs2_file_size(&_lfs, f);
    LFS2_TRACE("file_size -> %d", lfs2_toerror(res));
    _mutex.unlock();
    return lfs2_toerror(res);
}
...
int LittleFileSystem2::file_truncate(fs_file_t file, off_t length)
{
    lfs2_file_t *f = (lfs2_file_t *)file;
    _mutex.lock();
    LFS2_TRACE("file_truncate(%p)", file);
    int err = lfs2_file_truncate(&_lfs, f, length);
    LFS2_TRACE("file_truncate -> %d", lfs2_toerror(err));
    _mutex.unlock();
    return lfs2_toerror(err);
}
...
int LittleFileSystem2::dir_open(fs_dir_t *dir, const char *path)
{
    ...
    _mutex.lock();
    LFS2_TRACE("dir_open(%p, \"%s\")", *dir, path);
    int err = lfs2_dir_open(&_lfs, d, path);
    LFS2_TRACE("dir_open -> %d", lfs2_toerror(err));
    _mutex.unlock();
    if (!err) {
        *dir = d;
    } else {
...
int LittleFileSystem2::dir_close(fs_dir_t dir)
{
    ...
    _mutex.lock();
    LFS2_TRACE("dir_close(%p)", dir);
    int err = lfs2_dir_close(&_lfs, d);
    LFS2_TRACE("dir_close -> %d", lfs2_toerror(err));
    _mutex.unlock();
    delete d;
    return lfs2_toerror(err);
}
...
ssize_t LittleFileSystem2::dir_read(fs_dir_t dir, struct dirent *ent)
{
    ...
    struct lfs2_info info;
    _mutex.lock();
    LFS2_TRACE("dir_read(%p, %p)", dir, ent);
    int res = lfs2_dir_read(&_lfs, d, &info);
    LFS2_TRACE("dir_read -> %d", lfs2_toerror(res));
    _mutex.unlock();
    if (res == 1) {
...
void LittleFileSystem2::dir_seek(fs_dir_t dir, off_t offset)
{
    lfs2_dir_t *d = (lfs2_dir_t *)dir;
    _mutex.lock();
    LFS2_TRACE("dir_seek(%p, %ld)", dir, offset);
    lfs2_dir_seek(&_lfs, d, offset);
    LFS2_TRACE("dir_seek -> %s", "void");
    _mutex.unlock();
}
...
off_t LittleFileSystem2::dir_tell(fs_dir_t dir)
{
    lfs2_dir_t *d = (lfs2_dir_t *)dir;
    _mutex.lock();
    LFS2_TRACE("dir_tell(%p)", dir);
    lfs2_soff_t res = lfs2_dir_tell(&_lfs, d);
    LFS2_TRACE("dir_tell -> %d", lfs2_toerror(res));
    _mutex.unlock();
    return lfs2_toerror(res);
}
...
void LittleFileSystem2::dir_rewind(fs_dir_t dir)
{
    lfs2_dir_t *d = (lfs2_dir_t *)dir;
    _mutex.lock();
    LFS2_TRACE("dir_rewind(%p)", dir);
    lfs2_dir_rewind(&_lfs, d);
    LFS2_TRACE("dir_rewind -> %s", "void");
    _mutex.unlock();
}
